...
FTGDEF void
ftg__stopwatch_next(struct ftg_stopwatch_s *stopwatch);
...
#endif
    if (sizeof(int64_t) > sizeof(size_t) && file_len > FTG__SIZE_MAX)
    {
...
ftg__stopwatch_start(struct ftg_stopwatch_s *sw, const char *name, const char *bucket_name, size_t bucket_idx)
{
    ...
    sw->current_bucket = bucket_idx;
    ftg_strncpy(sw->time_names[bucket_idx], bucket_name, FTG_STRLEN_SHORT);
